...
App.controller('JanusController', ['$scope', 'HomeService','uiGridConstants',"$mdDialog", function($scope, HomeService,uiGridConstants,$mdDialog) {
    ...
    if(window.location.protocol === 'http:')
        server = "http://" + "192.168.2.30" + ":8088/janus";
    else
        server = "https://" +"192.168.2.30" + ":8089/janus";
                                            ...
                                            if(event === "joined") {
                                                ...
                                                if(msg["publishers"] !== undefined && msg["publishers"] !== null) {
                                                    ...
                                                    if(remoteFeed != null) {
                                                        ...
                                                        $('#videoremote_'+remoteFeed.rfdisplay).empty();
                                                    ...
                                                    var remoteFeed = null;
                                                    for(var i=1; i<$scope.peripheralDetails.length; i++) {
        ...
        janus.attach(
            {
                ...
                onremotestream: function(stream) {
                    ...
                    // No remote video yet
                    $('#videoremote_'+remoteFeed.rfdisplay).append('<video class="rounded centered" id="waitingvideo' + remoteFeed.rfindex + '" width=320 height=240 />');
                    $('#videoremote_'+remoteFeed.rfdisplay).append('<video class="rounded centered relative hide" id="remotevideo' + remoteFeed.rfindex + '" width="100%" height="100%" autoplay/>');
                    $('#videoremote_'+remoteFeed.rfdisplay).append(
